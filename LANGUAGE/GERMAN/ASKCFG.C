const char *grfxdescription[NUM_GRFXDRVS]=
 { "Der Modus sollte Åberall laufen\n",
   "Der hier ist nut fÅrr S3 ChipsÑtze und hat kleine Fehler.\n"\
...
void testscreen(int grfxdrv,int res) 
 ...
 if(ws_initgrfx(xs,ys,256,init.fontname))
  ...
  { printf("Huppsa. Kann Grafikmouse nicht initialisieren.\n"\
   ...
   case 'D': *d_ver=((*d_ver)&1)+1;
    if(*d_ver==2)
     printf("\nDevil ist jetzt fÅr Descent 2 konfiguriert.\n"\
      "Sie kînnen auch Descent 1 Level laden, aber\n"
      "die Textures werden vîllig durcheinander sein.\n");
    else
   ...
   case 'Q':
    ...
    if(*d_ver==2)
     { strcpy(missionpath,d2path); strcat(missionpath,"/missions");
       printf("BestÑtige Descent 2 version 1.0 in %s\n",d2path);
       *d_ver=d2_10_reg; }
...
{ 0x01, 69, 66, "vorherige Ecke" },
{ 0x00,101, 67, "nÑchste Ecke" },
